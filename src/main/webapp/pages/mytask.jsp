...
<body class="no-skin">
	...
	jQuery(function($) {
		...
		function load(){
			$.ajax({
				type:'get',
				data:{"studentId":$("#currentId").val()},
				url:"${pageContext.request.contextPath }/admin/choose/mytask",
				success:function(data){
					...
					if(data!=null&&data.length>0){
						...
						for(var i=0;i<data.length;i++){
							item+="<tr>"
							+"<td>"+data[i].task.id+"</td>"
							+"<td>"+data[i].task.name+"</td>"
							+"<td>"+data[i].task.description+"</td>"
							+"<td>"+data[i].task.limitime+"</td>"
							+"<td>"
						+"<div class=\"hidden-sm hidden-xs action-buttons\">"
						+"<a class=\"btn_delete\" id=\""+data[i].id+"\" href=\"javascript:void(0)\" name=\""+data[i].id+"\" oper=\"delete\">"
						+"<i class=\"fa fa-trash-o bigger-150\">取消</i>"
						+"</a>"
					+"</div>"
						+"</td>"
						+"</tr>"
							...
							function(){
								...
								obj.click(
									function(){
											...
											$.ajax({
												type:'get',
												url:"${pageContext.request.contextPath }/admin/choose/deleteAjax",
												async:false,
												data:{'id':id},
												success:function(data){
													if(data.code==1){
