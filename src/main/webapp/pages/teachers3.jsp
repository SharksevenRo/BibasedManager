							...
							<!-- PAGE CONTENT BEGINS -->
									...
									<div class="div form-group col-md-12">
										...
										<div class="col-md-10 input-icon input-icon-right">
											<select id="sec-teacher" name="receiver.id" >
		...
		function load(){
			...
			$.ajax({
				...
				success:function(data){
					...
					debugger;
					var content=$("#sec-teacher");
					var item="<option value=\"${sessionScope.user.id}\">发送给所有小组</option>";
					for(var i=0;i<data.length;i++){
						
						item+="<option value=\""+data[i].id+"\">"+data[i].name+"小组</option>";
					}
					content.html("");
					content.append(item);
				}
